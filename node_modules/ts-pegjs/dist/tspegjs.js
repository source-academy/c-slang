...
const prettierPluginTypescript = require("prettier/parser-typescript");
...
const peggy__namespace = /* @__PURE__ */ _interopNamespaceDefault(peggy);
const prettierPluginTypescript__namespace = /* @__PURE__ */ _interopNamespaceDefault(prettierPluginTypescript);
...
function isFunctionLike(node) {
  return node.isKind(tsMorph.ts.SyntaxKind.FunctionDeclaration) || node.isKind(tsMorph.ts.SyntaxKind.FunctionExpression) || node.isKind(tsMorph.ts.SyntaxKind.ArrowFunction);
}
function isLiteral(node) {
  if (!node) {
    return false;
  }
  return node.isKind(tsMorph.ts.SyntaxKind.ObjectLiteralExpression) || node.isKind(tsMorph.ts.SyntaxKind.StringLiteral) || node.isKind(tsMorph.ts.SyntaxKind.ArrayLiteralExpression) || node.isKind(tsMorph.ts.SyntaxKind.NumericLiteral);
}
...
function getInfoFromDiagnostic(d, file) {
  ...
  if (!parent.isKind(tsMorph.ts.SyntaxKind.TypeAliasDeclaration)) {
    throw new Error(
      `Parent of node is of kind "${parent.getKindName()}", not "TypeAliasDeclaration"`
    );
  }
  const typeBodyNode = parent.getTypeNodeOrThrow();
  if (!typeBodyNode.isKind(tsMorph.ts.SyntaxKind.UnionType)) {
    throw new Error(
...
function listUnionedIdentifiers(node) {
  if (node.isKind(tsMorph.ts.SyntaxKind.UnionType)) {
    return node.getTypeNodes().flatMap((n) => listUnionedIdentifiers(n));
  }
  if (node.isKind(tsMorph.ts.SyntaxKind.ParenthesizedType)) {
    return listUnionedIdentifiers(node.getTypeNode());
  }
  if (node.isKind(tsMorph.ts.SyntaxKind.TypeReference)) {
...
class TypeExtractor {
  constructor(grammar, options) {
    ...
    __publicField(this, "project", new tsMorph.Project({
      compilerOptions: {
        allowJs: true,
    ...
    __publicField(this, "formatter", (str) => {
      try {
        return prettier.format(str, {
          parser: "typescript",
          plugins: [prettierPluginTypescript__namespace]
        });
      } catch (e) {
    ...
    if (typeof grammar === "string") {
      grammar = peggy__namespace.generate(grammar, { output: "ast" });
    }
  ...
  _getTypeForAction(action) {
    ...
    func.getBodyOrThrow().getChildrenOfKind(tsMorph.ts.SyntaxKind.ReturnStatement).forEach((r) => {
    ...
    const finalType = file.addTypeAlias({
      ...
      type: returnType.getText(func, tsMorph.ts.TypeFormatFlags.NoTruncation)
    });
    let finalTypeNode = finalType.getTypeNodeOrThrow();
    const paramsToType = new Map(labelNames.map((l, i) => [uniqueTypeParam(i), l.type]));
    if (finalTypeNode.isKind(tsMorph.ts.SyntaxKind.TypeReference)) {
      ...
      finalTypeNode.forEachDescendant((c) => {
        if (c.isKind(tsMorph.ts.SyntaxKind.TypeReference)) {
