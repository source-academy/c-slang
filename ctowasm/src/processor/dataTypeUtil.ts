...
import { primaryVariableSizes } from "~src/common/utils";
 ...
 */
export function unpackDataType(dataType: DataType): PrimaryDataTypeMemoryObjectDetails[]  {
  let currOffset = 0;
  ...
  function recursiveHelper(dataType: DataType) {
    if (dataType.type === "primary") {
      ...
      const numElements = evaluateCompileTimeExpression(dataType.numElements).value;
      ...
      throw new ProcessingError(`unpackDataType(): Invalid data type to unpack: ${toJson(dataType)}`);
    }
  } 
