...
import { EnclosingLoopDetails, generateBlockLabel } from "~src/translator/loopUtil";
import { getReg2SetNode } from "~src/translator/memoryUtil";
...
import { createIntegerConst, createWasmBooleanExpression } from "~src/translator/util";
import { WasmBlock, WasmSelectionStatement } from "~src/translator/wasm-ast/control";
import { WasmExpression, WasmStatement } from "~src/translator/wasm-ast/core";
import { WasmConditionalExpression } from "~src/translator/wasm-ast/expressions";

export default function translateSwitchStatement(switchStatement: SwitchStatementP, enclosingLoopDetails?: EnclosingLoopDetails): WasmStatement {
  ...
  for (let i = switchStatement.cases.length - 1; i >= 0 ; --i) {
    currExpression = {
      ...
      condition: createWasmBooleanExpression(switchStatement.cases[i].condition),
  ...
  for (; i < switchStatement.cases.length; ++i) {
    currBlock = {
      ...
      body: [currBlock, ...switchStatement.cases[i].statements.map(statement => translateStatement(statement, newEnclosingLoopDetails))]
    }
  }
  ...
  currBlock = {
    ...
    body: [currBlock, ...switchStatement.defaultStatements.map(statement => translateStatement(statement, newEnclosingLoopDetails))]
  }
