...
describe("Test stringifyDataType() function", () => {
  ...
  test("Test 2 - const pointer to const signed int", () => {
    expect(stringifyDataType({type: "pointer", pointeeType: {type: "primary", primaryDataType: "signed int", isConst: true}, isConst: true})).toBe("const pointer to const signed int");
  })
})